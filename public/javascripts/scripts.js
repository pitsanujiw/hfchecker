function getDataFromGithub(username) {
  var url =
    "https://api.github.com/search/issues?per_page=1000&q=author:" +
    username +
    "+type:pr+-label:invalid+created:2016-10-01..2016-10-31";
    ...
    success: function(data, status, jqxhr) {
      tweetIt = function tweetIt() {
        var phrase =
          "I have completed " +
          data.items.length +
          " pull requests. Check your progress on hfchecker.stefanopy.xyz #hacktoberfest";
        var tweetUrl =
      ...
      if (status == "success") {
        ...
        if (data.total_count != 0) {
          total.html(
            "Total PR's: " +
              data.items.length +
              "  <button class='btn btn-info' onclick='tweetIt()'>Tweet it!</button>"
          );
        } else {
        ...
        if (data.total_count != 0) {
              ...
              success: function(data2, status2, jqxhr2) {
                results.append(
                  "<tr>" +
                    ...
                    "</td>" +
                    "<td>" +
                    (item.closed_at == null ? "open" : "closed") +
                    "</td>" +
                    "</tr>"
                );
          ...
          results.html(
            "<tr><td colspan='5' style='text-align: center'>No pull request made by the user.</td></tr>"
          );
          loading.css("display", "none");
        }
      } else {
        results.html(
          "<tr><td colspan='5' style='text-align: center'>No data to show</td></tr>"
        );
        loading.css("display", "none");
      }
    },
...
function clock() {
  var deadline = "Nov 1, 2018 00:00:00";
